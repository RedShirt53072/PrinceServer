...
import com.github.redshirt53072.growthsurvival.ench.EnchData;
...
public final class EnchConfig {
	...
	private static EnchData readVanilla(String path,ConfigManager manager) {
		...
		if(name == null) {
			...
			name = TextBuilder.quickBuild(ChatColor.GRAY,name);
		}
		//int
		...
		for(String key : manager.getKeys(TextBuilder.plus(path,".cantexist"), "ench")) {
			String rawEnch = manager.getString(TextBuilder.plus(path,".cantexist.",key));
			Ench en = Ench.getEnch(rawEnch);
			if(en == null) {
				manager.logWarning(TextBuilder.plus(path,".cantexist.",key), "の値が不正です");
				continue;
			}
			cantList.add(en);
		}
